...
This environment includes **Flox-aware templates** that make Qwen Code automatically use Flox for environment management in your projects.
...
```bash
# Copy Qwen context file
cp $FLOX_ENV_TEMPLATES/QWEN.md ~/projects/my-app/
...
The templates instruct Qwen Code to:
...
Qwen: [Detects CUDA task]
Qwen: [Runs: ./fetch-flox.sh cuda]
Qwen: [Reads FLOX.md for CUDA guidance]
Qwen: [Applies Flox CUDA patterns]
Qwen: Let me help you set up CUDA using Flox...
```

**Customization:**

Create a `.env` file in your project for project-specific settings (without secrets):

```bash
# .env (no secrets!)
OPENAI_MODEL=qwen3-coder-32b
```
